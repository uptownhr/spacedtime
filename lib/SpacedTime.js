...
class SpacedTime {
  ...
  recallAnswer(qna, remembered = false) {
    if (remembered) {
      ...
      console.log(
        chalk.green('Congrats, we are going to ask you again later'),
        chalk.green(askAgainTime.fromNow())
      );
    } else {
      ...
      qna.askAgainTime = askAgainTime;
      console.log(
        chalk.redBright('No worries, spacedtime will ask again sooner'),
        chalk.redBright(askAgainTime.fromNow())
      );
    }
  ...
  remove(index) {
      const exists = this.data[index]

      if (!exists) throw new Error("Invalid Index")
  ...
  update(index, {question, answer}) {
      ...
      if (!q) throw new Error("Invalid Index")
